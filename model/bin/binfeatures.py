...
from model.bin.res_parts import NormalBlock, Down, Up  # noqa:E402
...
class FeatureNetwork(nn.Module):
    def __init__(self, init_channels, down_levels, up_levels, block_size, binary):
        ...
        self.init_conv = nn.Sequential(nn.Conv2d(3, init_channels, 3, 1, 1),
                                       ...
                                       nn.ReLU())
        self.top_conv = NormalBlock(init_channels, init_channels, 3, stride=2, pad=1,
                                    binary=False, bn=False, activation='')
        self.down_1 = nn.Sequential(Down(1, 2, n=1, binary=False, bn=True, activation=activation,
                                         residual=False),
                                    NormalBlock(2, 2, 3, stride=1, pad=1, binary=False, bn=False, activation=''))
        self.down_2 = nn.Sequential(Down(2, 4, n=1, binary=False, bn=True, activation=activation,
                                         residual=False),
                                    NormalBlock(4, 4, 3, stride=1, pad=1, binary=False, bn=False, activation=''))
        self.down_3 = nn.Sequential(Down(4, 8, n=1, binary=False, bn=True, activation=activation,
                                         residual=False),
                                    NormalBlock(8, 8, 3, stride=1, pad=1, binary=False, bn=False, activation=''))
        self.up_1 = Up(12, 4, n=1, binary=False, bn=True, activation=activation,
                       residual=False)
        self.last_up = NormalBlock(4, 4, 3, stride=1, pad=1,
...
class PostFeatureNetwork(nn.Module):
    def __init__(self, channels_3d, layers_3d, growth_rate, max_disp, binary):
        ...
        activation = 'relu'
        # Left processing
        self.down_1 = Down(self.max_disp, self.max_disp, n=1, binary=False, bn=True, activation=activation)
        self.down_conv_1 = NormalBlock(self.max_disp, self.max_disp, ks=3, binary=False,
                                       bn=True, activation=activation)
        self.down_conv_2 = NormalBlock(self.max_disp, self.max_disp, ks=3, binary=False, bn=False, activation='')

        self.up_1 = Up(self.max_disp * 2, self.max_disp, n=2, binary=False, activation=activation,
                       bn=True, residual=False)
        self.last_conv = NormalBlock(self.max_disp, self.max_disp, ks=3, binary=False, bn=False, activation='')
...
def main(benchmark, tensorrt):
    # Print summary
    fsa = BinFeatures(max_disp=192).cuda()
